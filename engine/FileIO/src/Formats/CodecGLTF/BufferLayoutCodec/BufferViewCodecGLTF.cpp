...
namespace fileio
{
		...
		safety::SafeArray<safety::SafeByteArray*>* buffers)
	{
		...
		bufferViewResult->byteOffset = (uint64_t)bufferViewJSON->getFieldNumberOrDefault("byteOffset", 0.0f);
		bufferViewResult->byteLength = (uint64_t)bufferViewJSON->getFieldNumberOrDefault("byteLength", 0.0f);
		bufferViewResult->byteStride = (uint16_t)bufferViewJSON->getFieldNumberOrDefault("byteStride", 0.0f);

		if (JSON* numberJSON = bufferViewJSON->getField("buffer"))
		{
			int32_t bufferIndex = (int32_t)numberJSON->getNumberOrDefault(-1.0f);
